...
namespace Orts.Simulation.RollingStocks
{
    ...
    public class MSTSWagon : TrainCar
    {
        ...
        public float AbsWheelSpeed1MpS;
        ...
        float TenderWagonMaxWaterMassKG = 0;
        ...
        public virtual void Parse(string lowercasetoken, STFReader stf)
        {
            switch (lowercasetoken)
            {
                ...
                case "wagon(ortsnumberaxles": WagonNumAxles = stf.ReadIntBlock(null); if (WagonNumAxles == 1) WagonNumAxles = 2; break;
        ...
        {
            this.PullPushValue = Math.Abs(this.CouplerSlackM);
            if (Math.Abs(this.CouplerSlackM) > 0.0f)
            {
        ...
        public override void Update(float elapsedClockSeconds)
        {
            ...
            foreach (MSTSCoupling coupler in Couplers)
            {
                ...
                if (IsPlayerTrain && Simulator.Settings.BreakCouplers && !Simulator.PlayerCoupling) // Only break couplers on player trains
                {
                    ...
                    if (CouplerForceU < 0 && ImpulsForce2 > 30000f)
        ...
        // This section updates the special effects
        {
            ...
            if ((Train.CarSteamHeatOn && LocomotiveParameters.SteamHeatController.CurrentValue > 0) || (Train.CarSteamHeatOn && !IsPlayerTrain))
            {
                ...
                if (!IsPlayerTrain)
                {
                    HeatingSteamBoilerDurationS = 1.0f * 0.5f;
                    HeatingSteamBoilerVolumeM3pS = 1.5f * 0.5f;
                }
                else
                {
            ...
            if ((Train.CarSteamHeatOn && CarSteamHeatMainPipeSteamPressurePSI > 0) || (Train.CarSteamHeatOn && !IsPlayerTrain))
            {
                if (!IsPlayerTrain)
                {
                    SteamHoseLeakRateRandom = (float)Simulator.Random.Next(100) / 100.0f; 
                    SteamHoseLeakRateRandom = MathHelper.Clamp(SteamHoseLeakRateRandom, 0.5f, 1.0f); 
                }
        ...
        public void ToggleBrakeCarDeactivateCarOperationsWindow()
        {
            foreach (var car in Train.Cars)
            {
                var mstsWagon = car as MSTSWagon;
        ...
        public void ToggleDoorsLeft()
        {
            ...
            {//inform everyone else in the train
                ...
                if (Simulator.PlayerLocomotive == this && DoorsCycle == 1)
                {
