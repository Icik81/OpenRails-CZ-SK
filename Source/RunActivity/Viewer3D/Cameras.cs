...
namespace Orts.Viewer3D
{
    public abstract class Camera
    {
        ...
        public void NearPlaneUpdate()
        {
            ...
            if (AttachedCar != null)
            {
                ...
                float CameraInPlacePerimeter = AttachedCar.CarLengthM / 2.0f + 0.5f;
        ...
        {
            var speed = 2.5f * elapsedTime.RealSeconds;
            if (UserInput.IsDown(UserCommand.CameraMoveFast))
                speed *= 2 * SpeedFactorFastSlow;
            if (UserInput.IsDown(UserCommand.CameraMoveSlow))
                speed /= 1 * SpeedFactorFastSlow;
            return speed;
        }
    ...
    public class CabCamera : NonTrackingCamera
    {
        ...
        public override void HandleUserInput(ElapsedTime elapsedTime)
        {
            ...
            // Icik
            if ((Viewer.Simulator.GameWasRestored && !CabViewRestored) || (Viewer.Simulator.PlayerIsInCab && PlayerTrainName != Viewer.PlayerTrain.Name))
            {
                Reset();
